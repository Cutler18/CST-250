...
namespace VehicleClassLibrary.Tests
{
    public class StoreLogicTests
    {
        ...
        [Fact]
        public void AddVehicleToCart_ShouldAddVehicle_WhenValidVehicleIdIsGiven()
        {
            // Arrange: Create an instance of StoreLogic
            StoreLogic store = new StoreLogic();
        ...
        // Test the GetShopiingCart method that it returns an empty list when no vehicles are added
        [Fact]
        ...
        public void Checkout_ShouldReturnCorrectTotal_AndClearCart()
        {
            ...
            // (Allowing a small variance for business logic such as discounts, taxes, etc.)
            Assert.True(total >= (car1.Price + car2.Price) * 0.95m);
            Assert.True(total <= (car1.Price + car2.Price) * 1.05m);

            // Assert: Verify that the shopping cart is now empty
